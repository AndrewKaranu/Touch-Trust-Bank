    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        ...
        Model.getInstance().getViewFactory().getClientSelectedMenu().addListener((observableValue, oldVal, newVal) -> {
            ...
            switch (newVal){
                ...
                case CREDIT -> {
                    System.out.println("Switching to Credit view");
                    client_parent.setCenter(Model.getInstance().getViewFactory().getCreditAccountView());
                }
                // In the initialize method's switch statement
                case VOICE_ASSISTANT -> {
                    client_parent.setCenter(Model.getInstance().getViewFactory().getVoiceAssistantView());
                }
                default -> {
